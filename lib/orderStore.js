...
async function makePgBackend() {
  ...
  `);
  // Para bases creadas antes de sumar estos campos.
  await pool.query(`ALTER TABLE orders ADD COLUMN IF NOT EXISTS address TEXT NOT NULL DEFAULT ''`);
  await pool.query(`ALTER TABLE orders ADD COLUMN IF NOT EXISTS payment_method TEXT NOT NULL DEFAULT 'efectivo'`);
  ...
  return {
    ...
    async add(o) {
      const r = await pool.query(
        `INSERT INTO orders (customer_name, phone, delivery_method, address, payment_method, items, subtotal, tax, total)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`,
        [o.customer_name, o.phone, o.delivery_method, o.address || '', o.payment_method || 'efectivo', JSON.stringify(o.items), o.subtotal, o.tax, o.total]
      );
      return map(r.rows[0]);
    },
